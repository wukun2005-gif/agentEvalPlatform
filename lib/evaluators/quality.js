...
function evaluate({ llm_output, query }) {
  ...
  // 检查重复：同一行出现 3 次以上，或同一句子连续出现 3 次
  ...
  // 检测连续重复短语（同一句子连续出现 3 次以上）
  const sentences = llm_output.split(/[。！？\n]+/).filter((s) => s.trim().length > 5);
  for (let i = 0; i < sentences.length - 2; i++) {
    if (sentences[i] === sentences[i + 1] && sentences[i + 1] === sentences[i + 2]) {
      return {
        name: "quality",
        score: 0.2,
        passed: false,
        reason: `输出包含连续重复: "${sentences[i].slice(0, 30)}..." 连续出现 3 次`,
      };
    }
  }
